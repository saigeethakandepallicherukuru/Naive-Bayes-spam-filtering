			...
			if(not file.startswith('.')):
				...
				for line in fp.readlines():
					...
					for word in words:
						word = word.rstrip()
						if(len(word) != 0):
							prob_ham_class = Decimal(prob_ham_class) * Decimal((((ham_dict.get(word) if(ham_dict.get(word)) else 0)+1)) / Decimal((vocab.get("ham_token_count")+vocab.get("vocab"))))
							prob_spam_class = Decimal(prob_spam_class) * Decimal((((spam_dict.get(word) if(spam_dict.get(word)) else 0)+1)) / Decimal((vocab.get("spam_token_count")+vocab.get("vocab"))))
